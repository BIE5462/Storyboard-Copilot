...
export function FormToolEditor({ fields, options, onOptionsChange }: FormToolEditorProps) {
  ...
  const resolveLabel = useCallback(
    (label: string, labelKey?: string) => (labelKey ? t(labelKey) : label),
    [t]
  );
        ...
        >
          {field.options.map((option) => (
            <option key={option.value} value={option.value}>
              {resolveLabel(option.label, option.labelKey)}
            </option>
          ))}
        </UiSelect>
      );
    },
    [options, resolveLabel, t, updateOption]
  );
      ...
      {fields.map((field) => (
        <div key={field.key}>
          <label className="mb-1 block text-xs text-text-muted">
            {resolveLabel(field.label, field.labelKey)}
          </label>
          {renderField(field)}
        </div>
      ))}
